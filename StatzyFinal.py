...
# had to change how the code functioned to work with single line declarations and assignments
# this was a problem in the original code, as it would not work with single line declarations and assignments
...
# method overload to handle both immediate values and variable assignments
def setVariableRegister(varName, tRegister):
    global vars
    vars[varName] = tRegister
...
def getModuloChecklines(varName, divisor):
    global tRegister
    varReg = getVariableRegister(varName)

    regNum = tRegister
    output = f"lw S{regNum}, 0({varReg})\n"
    output += f"li S{regNum + 1}, {divisor}\n"
    output += f"rem S{regNum + 2}, S{regNum}, S{regNum + 1}\n"

    tRegister += 3
    return output, f"S{regNum + 2}"  
...
for line in lines:
    ...
    if line.startswith("if "):
        ...
        if "%" in expr and "==" in expr:
            ...
            if reg == "ERROR":
                ...
                if right == "0":
                    outputText += f"bne S{regNum + 2}, ZERO, exit\n"
                else:
                    outputText += f"# Unsupported comparison: {expr}\n"
                tRegister += 3

        else:
            # comparisons e.g x < 5, x > 5, x == 5, etc.
            ...
            if op == "<":
                outputText += f"bge {tempReg}, {constReg}, exit\n"
            elif op == "<=":
                outputText += f"bgt {tempReg}, {constReg}, exit\n"
            elif op == ">":
                outputText += f"ble {tempReg}, {constReg}, exit\n"
            elif op == ">=":
                outputText += f"blt {tempReg}, {constReg}, exit\n"
            elif op == "==":
                outputText += f"bne {tempReg}, {constReg}, exit\n"
            elif op == "!=":
                outputText += f"beq {tempReg}, {constReg}, exit\n"


    

    # int declarations (with or without =) — this was a problem in the original code
    elif line.startswith("int ") and "(" not in line and ")" not in line:
        declaration = line.replace("int", "").strip().strip(";")
        if "=" in declaration:
            varName, val = declaration.split("=")
            varName = varName.strip()
            val = val.strip()

            outputText += getInstructionLine(varName) + "\n"

            if "+" in val:
                var1, var2 = val.split("+")
                var1 = var1.strip()
                var2 = var2.strip()
                outputText += getAdditionLines(var1, var2, varName) + "\n"
            elif val.isdigit():
    ...
    # assignments (x = 5 or x = y or x = a + b)
    elif "=" in line:
        assignment = line.strip().strip(";")
        if "+" in assignment:
            varName, expr = assignment.split("=")
            ...
            varName, _, val = assignment.split()
            if val.isdigit():
                outputText += getAssignmentLinesImmediateValue(val, varName) + "\n"
            else:
                outputText += getAssignmentLinesVariable(val, varName) + "\n"
...
# FizzBuzz logic appended from STATzyFizzbuzz.py
